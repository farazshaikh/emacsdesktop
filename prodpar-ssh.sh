...
parallel_ssh() {
    ...
    tmux new-window -t ${SESSION} "SSH_AUTH_SOCK=$(gpgconf --list-dirs agent-ssh-socket); ssh ${hosts[0]}"
    #tmux new-window -t ${SESSION} "ssh ${hosts[0]}"
    unset hosts[0];

    #split remaining sessions in the same window
    for i in "${hosts[@]}"; do
	    tmux split-window -t ${SESSION} -h  "SSH_AUTH_SOCK=$(gpgconf --list-dirs agent-ssh-socket); ssh $i"
